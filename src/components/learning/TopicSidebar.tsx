...
function FileItem({
  ...
  const href = `/dashboard/${topicSlug}/${file.slug}`;
  return (
...
function GroupSection({
  ...
  const isActive = group.files.some(
    (f) => pathname === `/dashboard/${topicSlug}/${f.slug}`
  );
  ...
  return (
    <div className="mb-1 mt-3 first:mt-0">
      ...
      {/* Group files */}
      {open && (
        <SidebarMenu className="ml-3 border-l border-border pl-2 mt-2 group-data-[collapsible=icon]:hidden">
...
export function TopicSidebar({ topic }: { topic: TopicItem }) {
  ...
  const isGroupActive = (group: TopicGroup) =>
    group.files.some(
      (f) => pathname === `/dashboard/${topic.slug}/${f.slug}`
    );
            ...
            {topic.files.length > 0 && (
              <SidebarMenu>
                {topic.files.map((file) => {
                  const href = `/dashboard/${topic.slug}/${file.slug}`;
