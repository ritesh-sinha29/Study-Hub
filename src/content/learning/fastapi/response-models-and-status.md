...
<Questions>
<Question id="Q1" title="Why should we use `response_model` in FastAPI?">
Three key reasons:
...
<Question id="Q2" title="What is the difference between HTTP 200, 201, 204?">
200 OK       → Request succeeded. Data returned in body. Used for GET, PUT.
...
<Question id="Q3" title="What is the difference between HTTP 401 and 403?">
401 Unauthorized → "Who are you?" — Client is NOT authenticated (no/invalid token).
...
<Question id="Q4" title="How does FastAPI's `response_model` filter data?">
When you return a dict or ORM object, FastAPI:
...
<Question id="Q5" title="What is `status.HTTP_201_CREATED` and why use it instead of just writing `201`?">
`status.HTTP_201_CREATED` is a constant from FastAPI that equals the integer 201.
