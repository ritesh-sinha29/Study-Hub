...
<Questions>
<Question id="Q1" title="What is Dependency Injection and why is it used in FastAPI?">
Dependency Injection (DI) means: a function declares what it NEEDS (its dependencies)
...
<Question id="Q2" title="How does `Depends()` work in FastAPI?">
When FastAPI sees `param = Depends(some_function)`, it:
...
<Question id="Q3" title="What is the difference between `return` and `yield` in a FastAPI dependency?">
`return` → Simple dependency. Runs setup, returns value, done.
...
<Question id="Q4" title="Can one dependency depend on another dependency in FastAPI?">
YES! FastAPI resolves nested/chained dependencies automatically.
Example chain:
...
<Question id="Q5" title="How do you override a dependency in tests?">
FastAPI provides `app.dependency_overrides` for this.
Example:
