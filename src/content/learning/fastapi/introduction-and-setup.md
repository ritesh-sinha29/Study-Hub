...
<Questions>
<Question id="Q1" title="What is FastAPI and why is it faster than Flask or Django REST?">
...
<Question id="Q2" title="What is the difference between WSGI and ASGI?">
WSGI (Web Server Gateway Interface) → Synchronous. One request blocks until complete.
...
<Question id="Q3" title="What is Uvicorn and why do we need it?">
Uvicorn is an ASGI server — it's the "web server" that actually listens for HTTP connections.
...
<Question id="Q4" title="What is OpenAPI and how does FastAPI use it?">
OpenAPI (formerly Swagger) is a standard specification for describing REST APIs.
...
<Question id="Q5" title="How does FastAPI handle async functions vs regular functions?">
`async def` → FastAPI runs it in the async event loop. Perfect for DB calls, HTTP calls.
