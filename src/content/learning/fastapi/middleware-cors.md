...
<Questions>
<Question id="Q1" title="What is middleware in FastAPI and how is it different from a route?">
A route handles ONE specific URL (e.g., GET /users).
...
<Question id="Q2" title="What is CORS and why does it exist?">
...
<Question id="Q3" title="What is the difference between allow_origins=["*"] and allow_origins=["https://myapp.com"]?">
allow_origins=["*"] → ALLOWS ALL origins. Any website can call your API.
...
<Question id="Q4" title="What is the `call_next` function inside a middleware?">
`call_next(request)` is a function that passes the request to the NEXT handler
...
<Question id="Q5" title="How would you add an API key check to ALL routes using middleware?">
@app.middleware("http")
