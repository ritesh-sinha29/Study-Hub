...
<Questions>
<Question id="Q1" title="What is JWT and how does it work?">
JWT = JSON Web Token. It's a compact, signed string with 3 parts separated by dots:
...
<Question id="Q2" title="What is the difference between Authentication and Authorization?">
Authentication → "Who are you?" — Verifying identity. (Login with username/password)
...
<Question id="Q3" title="Why do we hash passwords instead of storing them in plain text?">
If your database is compromised (hacked), plain text passwords are exposed directly.
...
<Question id="Q4" title="What is OAuth2 and how does FastAPI implement it?">
...
<Question id="Q5" title="What is token expiration and why is it important?">
JWT tokens have an `exp` (expiration) claim. After this time, the token is INVALID.
Why important:
