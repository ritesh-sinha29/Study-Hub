...
<Questions>
<Question id="Q1" title="What is HTTPException in FastAPI and when do you use it?">
`HTTPException` is a special exception class from FastAPI.
...
<Question id="Q2" title="What is the difference between HTTPException and a regular Python Exception?">
Regular Python Exception → Used in business logic. Has no HTTP concept.
...
<Question id="Q3" title="What are the most important HTTP error status codes to know?">
...
<Question id="Q4" title="What is an exception_handler in FastAPI and why is it useful in production?">
@app.exception_handler(SomeException) registers a GLOBAL catcher for that exception.
Why useful:
...
<Question id="Q5" title="What is the difference between status 400 and 422 in FastAPI?">
400 Bad Request  → YOU raise manually when business logic fails.
