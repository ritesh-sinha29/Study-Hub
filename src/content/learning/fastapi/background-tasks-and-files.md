...
<Questions>
<Question id="Q1" title="What are BackgroundTasks in FastAPI? When should you use them?">
BackgroundTasks let you run a function AFTER the HTTP response is sent to the client.
...
<Question id="Q2" title="What is the difference between BackgroundTasks and asyncio tasks in FastAPI?">
BackgroundTasks → Runs AFTER the response is sent. Single server. Simple to use.
...
<Question id="Q3" title="What is UploadFile in FastAPI and why is it better than `bytes`?">
`file: bytes` → Reads the ENTIRE file into RAM at once.
...
<Question id="Q4" title="What library must be installed to handle file uploads in FastAPI?">
...
<Question id="Q5" title="How does FastAPI's BackgroundTasks differ from Celery? When would you use Celery at an MNC?">
BackgroundTasks:
