...
<Questions>
<Question id="Q1" title="What is an ORM and why is SQLAlchemy used in FastAPI projects?">
ORM (Object Relational Mapper) lets you interact with a database using Python objects
...
<Question id="Q2" title="What is the difference between `db.add()`, `db.commit()`, and `db.refresh()`?">
db.add(obj)     → Stages the object for insertion (like git add). NOT in DB yet.
...
<Question id="Q3" title="What is the difference between SQLite and PostgreSQL? When do you use which?">
SQLite:
...
<Question id="Q4" title="What does `from_attributes = True` in Pydantic's Config class do?">
By default, Pydantic models only accept dictionaries as input.
...
<Question id="Q5" title="What is the purpose of a `get_db` dependency with `yield` in FastAPI?">
It follows the "open-use-close" pattern for database connections:
