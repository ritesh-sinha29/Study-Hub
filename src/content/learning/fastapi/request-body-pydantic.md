...
<Questions>
<Question id="Q1" title="What is Pydantic and why does FastAPI use it?">
Pydantic is a data validation library that uses Python type hints to define data schemas.
...
<Question id="Q2" title="What is the difference between a Pydantic model and a SQLAlchemy model?">
...
<Question id="Q3" title="What is the `Field` class in Pydantic? Give examples.">
`Field` adds extra rules and metadata to a Pydantic model attribute.
Common uses:
...
<Question id="Q4" title="How does FastAPI know which parameter is a path param, query param, or request body?">
FastAPI uses a clear priority system:
...
<Question id="Q5" title="What does `model_dump()` do in Pydantic v2?">
