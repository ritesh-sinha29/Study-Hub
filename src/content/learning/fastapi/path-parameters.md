...
<Questions>
<Question id="Q1" title="What are path parameters in FastAPI? Give a real-life example.">
Path parameters are dynamic parts of the URL. FastAPI captures them using `{variable_name}`.
...
<Question id="Q2" title="What happens when you access /items/abc in a route defined as /items/{item_id} where item_id: int?">
...
<Question id="Q3" title="Why must static routes be defined BEFORE dynamic routes?">
FastAPI matches routes TOP TO BOTTOM in the file.
...
<Question id="Q4" title="What is the difference between `item_id: int` and `item_id: str` as a path param?">
`item_id: int` → FastAPI validates the value is a valid integer. /items/abc → 422 error.
...
<Question id="Q5" title="What does {file_path:path} mean in FastAPI routes?">
By default, path parameters don't match forward slashes (/).
