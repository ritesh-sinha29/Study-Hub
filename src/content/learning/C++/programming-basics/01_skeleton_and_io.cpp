...
//   * Console Applications: Command-line utilities that read user inputs and
//   print output.
...
// <iostream> contains declarations for standard input/output streams (cin,
// cout).
#include <iostream>

// 'using namespace std' allows us to use elements in the 'std' namespace
// without the 'std::' prefix.
using namespace std;

// The main() function is the mandatory entry point where execution begins.
int main() {
  // 'cout' prints text to the screen.
  // 'endl' inserts a newline and flushes the stream.
  cout << "Hello, Sinha!" << endl; // Output: Hello, Sinha!

  // '\n' is also used for a newline. It is generally faster than 'endl' because
  // it doesn't force a stream flush.
  ...
  // 'cin' reads input from the user keyboard.
  ...
  // You can chain 'cin' to read multiple values separated by spaces or newlines.
  cin >> x >> y; // (Assume user inputs: 10 12)
  cout << "Value of x: " << x << " and y: " << y << endl; // Output: Value of x: 10 and y: 12

  // --- The Master Header: <bits/stdc++.h> ---
  // In competitive programming, instead of writing multiple #include
  // statements, developers often write: #include <bits/stdc++.h> This header
  // includes every standard library header automatically, saving time.

  // Return 0 indicates to the operating system that the program executed
  // successfully.
  return 0;
}
...
// A:  Both insert a newline. However, 'std::endl' also flushes the output
// buffer
//     (writes immediately to the screen/file), which is a slow operation.
//     '\n' just inserts the newline character without flushing, making it much
//     faster.
//
// ⭐ Q2. Why is 'using namespace std;' considered bad practice in large
// projects? A:  It imports the entire standard namespace into the global
// namespace. This
...
//     should be avoided in production environments because it increases
//     compilation times and isn't supported by all compilers.
